...
typedef NS_ENUM(NSUInteger, PCControlState) {
    ...
    PCControlStateSelected     = 1 << 3
};


/**
 PCControl is the abstract base class of the Cocos2d components that handles touches or mouse events. You cannot instantiate it directly, instead use one of its sub-classes, such as CCButton. If you need to create a new sort of component you should make a sub-class of this class.
 
 The control class handles events and its sub-classes will use child nodes to draw itself in the node heirarchy.
 
 *Important:* If you are sub-classing PCControl you will need to include the PCControlSubclass.h file as it includes methods that are otherwise not exposed.
 */
@interface PCControl : SKSpriteNode
