...
- (void)keyDown:(NSEvent *)theEvent {
    ...
    if (key == NSDeleteCharacter) {
        ...
        PCTemplate *templ = [[self content] objectAtIndex:idx];
...
- (void)mouseDown:(NSEvent *)theEvent {
    [AppDelegate appDelegate].nodeIsBeingCreatedFromShortCutCollectionViewPane = YES;
    [super mouseDown:theEvent];
    NSInteger index = [[self selectionIndexes] firstIndex];
    if (index == NSNotFound) return;
    PCTemplate *template = [[self content] objectAtIndex:index];

    // Check for click count above one, which we assume means it's a double click
    if ([theEvent clickCount] > 1) {
        if ([self selectionIndexes].count) {
            [[AppDelegate appDelegate].propertyInspectorHandler applyTemplate:template];
        }
    }

    if (self.isShortCutSelection) {
        NSDictionary *nodeInfo = @{ @"particleTemplate" : template };
        [[AppDelegate appDelegate] dropAddPlugInNodeNamed:@"PCParticleSystem" userInfo:nodeInfo];
        [[AppDelegate appDelegate].selectedPopover performClose:nil];
