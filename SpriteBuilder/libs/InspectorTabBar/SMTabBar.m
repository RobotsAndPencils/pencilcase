...
+ (CGFloat)SMTabBarButtonWidth {
    static CGFloat buttonWidth = 32.0f;
    return buttonWidth;
}

+ (Class)buttonCellClass {
    return [SMTabBarButtonCell class];
}
...
- (void)updateButtons {
	...
	for (SMTabBarItem *item in self.items) {
		NSButton *button = [[NSButton alloc] initWithFrame:NSMakeRect(0.0f, 0.0f, [[self class] SMTabBarButtonWidth], NSHeight(self.bounds))];
		
		// add special button cell for the selected state
		button.cell = [[[[self class] buttonCellClass] alloc] init];
        ...
        button.imagePosition = NSImageBelow;
        button.font = [NSFont systemFontOfSize:[NSFont systemFontSizeForControlSize:NSMiniControlSize]];
...
- (void)adjustSubviews {
    ...
    CGFloat completeWidth = numberOfButtons * [[self class] SMTabBarButtonWidth];
    CGFloat offset = floorf((NSWidth(self.bounds) - completeWidth) / 2.0f);
    for (NSButton *button in self.barButtons) {
        button.frame = NSMakeRect(offset, NSMinY(self.bounds), [[self class] SMTabBarButtonWidth], NSHeight(self.bounds));
        offset += [[self class] SMTabBarButtonWidth];
    }
}
